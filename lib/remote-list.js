...
var CONFIG_FILE = path.normalize(path.join(config.home, config.remotesFile));
var remotes = [];

//module.exports = exports = emitter;

var remoteToString = function (fmt) {
    var result = this.name || this.host;
    switch(fmt) {
        case 'long': 
            result += (this.source !== 'user' ? [' (', this.source, ')'].join('') : '');
    };
...
exports.add = function (remote, callback, persist) {
    if (!~remotes.indexOf(remote)) {
        remotes.push(remote);
        remote.toString = remoteToString;
